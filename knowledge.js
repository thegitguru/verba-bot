const VERBA_KNOWLEDGE = {
  syntax: [
    ...
    { topic: "Variables & Strings", keywords: ["variable", "variables", "assignment", "constant", "immutability", "string", "boolean", "null", "interpolation"], description: "Variables are dynamic. Use `constant` for immutable names that cannot be reassigned. Double-quoted strings support `{value}` interpolation, while single-quoted strings stay raw.", example: "name = \"Alice\".\nconstant PI = 3.14.\nis_admin = true.\nsay \"Hello, {name}!\"." },
    { topic: "Null Safety & Option Types", keywords: ["option", "options", "null safety", "nullable", "optional", "some", "none", "unwrap", "unwrap_or", "or_else", "map", "filter", "and_then"], description: "Verba supports first-class option values with `some <expr>` and `none`. Use `is some` / `is none` in conditions, then safely transform values with `map`, `filter`, and `and_then`, or read them with `unwrap`, `unwrap_or`, and `or_else`.", example: "name = some \"Verba\".\nmissing = none.\nif name is some:\n    res = the result of running name.map with double.\nend.\nfallback = the result of running missing.unwrap_or with \"Anonymous\"." },
    ...
    { topic: "Collections", keywords: ["list", "lists", "map", "maps", "dictionary", "array", "comprehension", "slice", "sort", "functional", "filter", "reduce", "any", "all", "sum", "join", "keys", "values", "get", "has"], description: "Lists and Maps support powerful functional methods: `map`, `filter`, `reduce`, `any`, `all`, `sum`, `join`, `count`. Maps also include `keys`, `values`, `get`, and `has` for safe access.", example: "nums = [1, 2, 3].\ndoubled = the result of running nums.map with square.\ntotal = the result of running nums.sum.\nprices = {\"Apple\": 10}.\ncost = the result of running prices.get with \"Apple\", 0." },
    ...
    { topic: "Pattern Matching", keywords: ["match", "when", "pattern", "destructure", "switch", "wildcard", "type check", "is"], description: "Use `match` and `when` for value-based control flow. Supports literal matches, wildcards (`_`), type checks (`is number`, `is word`, `is flag`, etc.), and structure matches for lists and maps.", example: "match val:\n    when is number: say \"Digit\".\n    when [1, _, 3]: say \"List pattern\".\n    when _: say \"Wildcard\".\nend." },
    { topic: "Pointers & Memory", keywords: ["pointer", "reference", "references", "deref", "memory", "free", "delete variable", "constant"], description: "References use `&name`, dereferencing uses `deref ptr`, and variables or constants can be removed from scope with `free` or `delete`.", example: "x = 42.\nptr = &x.\nderef ptr = 100.\nfree ptr.\nconstant C = 10.\nfree C." },
    ...
    { topic: "Async & Await", keywords: ["async", "await", "task", "background", "concurrency"], description: "Async functions use `async define`, run with `async run`, and resolve with `await result = task`. These run within the same OS process.", example: "async define fetch_data as follows:\n    give \"done\".\nend.\ntask = async run fetch_data.\nawait result = task." },
    { topic: "Parallelism", keywords: ["parallel", "join", "multiprocessing", "process", "concurrency", "cpu"], description: "Parallel functions run in a separate OS process via `parallel run ... and save to ...`. Use `join ... = ...` to wait and retrieve the result. Best for CPU-heavy tasks.", example: "parallel run calc with 1000 and save to task_p.\njoin result = task_p." },
    { topic: "Modules, Help & Testing", keywords: ["import", "module", "modules", "help", "test", "assert", "with statement", "decorator"], description: "Verba supports flexible `.vrb` imports (built-in or relative), inline help, built-in tests with `test` and `assert`, `with` blocks for scoped resources, and decorators such as `@log` and `@time`.", example: "import math.\nimport my_lib as L.\nimport from \"utils.vrb\" as u.\ntest \"math works\":\n    assert 1 == 1.\nend." }
  ],
  stdlib: {
    strings: [
  ...
  examples: [
    ...
    { name: "examples/options.vrb", desc: "Option values with some/none, unwrap, unwrap_or, and null-safety checks." },
...
function buildFollowups(hits, query) {
  ...
  if (query.toLowerCase().includes("parallel")) suggestions.push("parallelism example");
